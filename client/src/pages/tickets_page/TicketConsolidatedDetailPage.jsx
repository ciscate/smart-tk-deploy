...
import {
    getTicketConsolidated,
    deleteTicketConsolidated,
    updateTicketConsolidated,
} from "../../api/tickets_api/ticketsconsolidated.api";
import { useNavigate, useParams } from "react-router-dom";
import { useForm } from "react-hook-form";
import { getStaffUsers, getUser } from "../../api/users_api/usersapi";
import { toast } from "react-hot-toast";
import { TicketConsolidatedEdit } from "../../components/tickets_components/TicketConsolidatedEdit";
import { ConfirmModal } from "../../components/DeleteModalConfirm"; // Importa el modal de confirmación
import consolidatedIcon from "../../assets/consolidated.png";
import { getCurrentUserData } from "../../api/users_api/usersapi";
import * as XLSX from 'xlsx';

export function TicketConsolidatedDetailPage() {
    const { id } = useParams();
    const {
        register,
        handleSubmit,
    ...
    const [isConfirmOpen, setIsConfirmOpen] = useState(false); // Estado para controlar el modal
    const [assignedStaff, setAssignedStaff] = useState(null);
    const [selectedStaff, setSelectedStaff] = useState("");
    const [status, setStatus] = useState("");
    const [newStatus, setNewStatus] = useState(""); // Inicializar vacío
    const [userData, setUserData] = useState(null);
    const [canEdit, setCanEdit] = useState(false); // Estado para verificar permisos
    ...
    useEffect(() => {
        if (id) {
            async function loadTicketConsolidated() {
                try {
                    const response = await getTicketConsolidated(id);
                    ...
                    setSelectedStaff(
                        response.data.assigned_to ? response.data.assigned_to.id : ""
                    );
                    setStatus(response.data.status); // Actualiza el estado del ticket
                    setNewStatus(response.data.status); // Sincroniza newStatus con el estado real del ticket
                } catch (error) {
                    console.error("Error loading ticket:", error);
                }
            }
            loadTicketConsolidated();
        }
    }, [id, setValue]);
    ...
    const onSubmit = handleSubmit(async (formData) => {
        ...
        try {
            const response = await updateTicketConsolidated(id, data, {
    ...
    // Función para obtener el nombre y la extensión de un archivo
    const getFileNameAndExtension = (fileName) => {
        const parts = fileName.split(".");
        const extension = parts.length > 1 ? parts.pop() : "Unknown";
        let name = parts.join(".");
        name = name.split("_")[0];
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteTicketConsolidated(id);
                ...
                {ticket ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex flex-col items-center justify-center space-y-6">
                            ...
                            <div className="bg-green-200 p-6 rounded-lg border border-gray-400 text-gray-800 w-full shadow">
                                ...
                                <p className="text-2xl">
                                    Periodo: {ticket.period_start} a {ticket.period_end}
                                </p>
                                <p className="text-2xl">
                                    Usuario: {ticket.user.name} {ticket.user.surname}
                                </p>
                                <p className="text-2xl">Department: {ticket.user.department}</p>
                                    ...
                                    <div className="flex flex-col space-y-2 ">
                                        ...
                                        {userData.is_superuser || userData.is_staff ? (
                                            <textarea
                                                ...
                                                className="w-full p-4 border border-green-300 rounded-lg bg-gray-50 text-xl resize-none focus:outline-none focus:ring-2 focus:ring-green-300"
                                                rows="4"
                                            ...
                                            <div
                                                id="observation"
                                                className="w-full p-4 border border-green-300 rounded-lg bg-gray-50 text-xl"
                                            >
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                ...
                                                {userData.is_superuser || userData.is_staff ? (
                                                    <select
                                                        ...
                                                        className="w-full p-4 border border-green-300 rounded-lg bg-gray-50 text-xl text-center focus:outline-none focus:ring-2 focus:ring-green-300"
                                                    >
