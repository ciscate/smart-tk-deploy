...
import { createTicketConsolidated } from "../../api/tickets_api/ticketsconsolidated.api";
...
export function TicketConsolidatedFormPage() {  
    const { 
        register, 
        handleSubmit, 
        formState: { errors }, 
    ...
    const onSubmit = handleSubmit(async (data) => {
            ...
            try {
                await createTicketConsolidated(formData, {
    ...
    const handleAddClient = (cliente) => {
            ...
            if (!selectedClients.some((c) => c.cuit_acceso === cliente.cuit_acceso)) {
                ...
                console.log("Clientes seleccionados actualmente:", updatedClients); // Mostrar en consola
                toast.success(`Cliente ${cliente.razon_social} agregado/a`);
            } else {
                console.log(`El cliente ${cliente.razon_social} ya está agregado/a`);
    ...
    const removeClient = (index) => {
            ...
            return (
                <div className="flex items-start mx-auto w-3/5 justify-between min-h-screen bg-green-100 p-6 rounded-lg mt-12">
                    {/* Sección de búsqueda */}
                    <div className="w-2/4 bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300 space-y-5 mr-6">
                        ...
                        <input
                            ...
                            className="w-full p-3 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 text-xl"
                        />
                            ...
                            {searchTerm &&
                                filteredClients.map((cliente, index) => (
                                    ...
                                    >
                                        ...
                                        {/* Botón "+" */}
                                        <button
                                            className="flex items-center justify-center w-10 h-10 rounded-full bg-red-500 text-white font-bold text-xl hover:bg-red-600 transition duration-300"
                        ...
                        <form onSubmit={onSubmit} className="space-y-6">
                            ...
                            <div>
                                    ...
                                    <input 
                                        ...
                                        multiple
                                        className="w-full p-3 border rounded-full bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 text-gray-700"
                                    />
                                </div>
                                <div className="mt-4">
                                                ...
                                                {selectedFiles.map((file, index) => (
                                                    <li key={index} className="text-gray-600 flex justify-between items-center text-lg my-2">
                                                        ...
                                                        <button 
                                                            ...
                                                            className="ml-4 bg-green-500 text-white font-bold rounded-full w-8 h-8 flex items-center justify-center hover:bg-green-600"
                                                        >
                                                            X
                                                        </button>
                                                    </li>
                            ...
                            {/* Botón de guardar */}
                            <button className="border border-green-500 text-green-500 p-3 rounded-lg w-full hover:bg-green-500 hover:text-white transition duration-300">
                                GUARDAR
                            </button>
                        </form>
                    </div>
