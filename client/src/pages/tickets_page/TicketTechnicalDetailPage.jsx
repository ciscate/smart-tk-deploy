...
import {
    getTicketTechnical,
    updateTicketTechnical,
    deleteTicketTechnical,
} from "../../api/tickets_api/ticketstechnical.api";
import { useNavigate, useParams } from "react-router-dom";
import { getStaffUsers } from "../../api/users_api/usersapi";
import { useForm } from "react-hook-form";
import { toast } from "react-hot-toast";
import { TicketTechnicalEdit } from "../../components/tickets_components/TicketTechnicalEdit";
...
export function TicketTechnicalDetailPage() {
    const { id } = useParams();
    const {
        register,
        handleSubmit,
    ...
    const [selectedStaff, setSelectedStaff] = useState(""); // Agregar el estado para selectedStaff
    const [status, setStatus] = useState(""); // Estado para el estado del ticket
    const [newStatus, setNewStatus] = useState(ticket ? ticket.status : "Enviado"); // "Sent" como valor predeterminado
    const [userData, setUserData] = useState(null); // Declarar userData en el estado
    ...
    useEffect(() => {
        if (id) {
            async function loadTicketTechnical() {
                try {
                    const response = await getTicketTechnical(id);
                    ...
                    setNewStatus(response.data.status); // Inicializa el estado con el estado actual del ticket
                } catch (error) {
    ...
    const onSubmit = handleSubmit(async (formData) => {
        ...
        try {
            const response = await updateTicketTechnical(id, data, {
                headers: { "Content-Type": "multipart/form-data" },
            });

            if (response.status === 200) {
                toast.success("Ticket se actualizo con exito", {
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteTicketTechnical(id);
            toast.success("Ticket se actualizo con exito", {
                position: "bottom-right",
                style: {
                    background: "#101010",
                ...
                {ticket ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex flex-col items-center justify-center space-y-6">
                            ...
                            <div className="bg-red-200 p-6 rounded-lg border border-gray-400 text-gray-800 w-full shadow">
                                    ...
                                    <div className="flex flex-col space-y-2 ">
                                        ...
                                        {userData.is_superuser || userData.is_staff ? (
                                            <textarea
                                                ...
                                                className="w-full p-4 border border-red-300 rounded-lg bg-gray-50 text-xl resize-none focus:outline-none focus:ring-2 focus:ring-red-300"
                                                rows="4"
                                            ...
                                            <div
                                                id="observation"
                                                className="w-full p-4 border border-red-300 rounded-lg bg-gray-50 text-xl"
                                            >
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                ...
                                                {userData.is_superuser || userData.is_staff ? (
                                                    <select
                                                        ...
                                                        className="w-full p-4 border border-red-300 rounded-lg bg-gray-50 text-xl text-center focus:outline-none focus:ring-2 focus:ring-red-300"
                                                    >
                                                    ...
                                                    <div
                                                        id="status"
                                                        className="w-full p-4 border border-red-300 rounded-lg bg-gray-50 text-xl text-center"
                                                    >
                                                        {newStatus}{" "}
