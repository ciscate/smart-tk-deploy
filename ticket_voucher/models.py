...
class TicketVoucher(models.Model):
    STATUS_OPTIONS = [
        ('Enviado', 'Enviado'),
        ('Leido', 'Leido'),
    ...
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='assigned_voucher_staff', on_delete=models.SET_NULL)
    ...
    # Nuevo campo para el período entre dos fechas
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.custom_id:
            self.custom_id = f"VOU-{self.pk}" if self.pk else f"VOU-TEMP-{timezone.now().strftime('%Y%m%d%H%M%S')}"
        if not self.status:
            self.status = 'Enviado'
        ...
        if self.custom_id.startswith("VOU-TEMP") and self.pk:
            self.custom_id = f"VOU-{self.pk}"
...
class TicketVoucherFile(models.Model):
    fileupload = models.FileField(upload_to='ticketsfiles/', validators=[validate_file_extension])
    ticket = models.ForeignKey(TicketVoucher, related_name='files', on_delete=models.CASCADE)
