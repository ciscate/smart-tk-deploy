...
from .serializer import TicketVoucherSerializer
...
class TicketVoucherView(viewsets.ModelViewSet):
    serializer_class = TicketVoucherSerializer
    queryset = TicketVoucher.objects.all()
    ...
    def user_tickets(self, request):
        user_tickets = TicketVoucher.objects.filter(user=request.user)
    ...
    def assigned_tasks(self, request):
        ...
        tasks = TicketVoucher.objects.filter(assigned_to=user)

        serializer = TicketVoucherSerializer(tasks, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
