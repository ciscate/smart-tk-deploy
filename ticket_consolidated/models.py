...
def validate_file_extension(file):
    ...
    allowed_mime_types = [
        ...
        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'  # XLSX (Excel nuevo)
    ]
...
class TicketConsolidated(models.Model):
    ...
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='assigned_consolidated_staff', on_delete=models.SET_NULL)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.custom_id:
            self.custom_id = f"CON-{self.pk}" if self.pk else f"CON-TEMP-{timezone.now().strftime('%Y%m%d%H%M%S')}"
        if not self.status:
            self.status = 'Enviado'
        ...
        if self.custom_id.startswith("CON-TEMP") and self.pk:
            self.custom_id = f"CON-{self.pk}"
    ...
    def __str__(self):
        return f"ID: {self.custom_id} - {self.title} - by {self.user.name if self.user else 'Unknown User'} | Status -> {self.status}"

class TicketConsolidatedFile(models.Model):
    fileupload = models.FileField(upload_to='ticketsfiles/', validators=[validate_file_extension])
    ticket = models.ForeignKey(TicketConsolidated, related_name='files', on_delete=models.CASCADE)
