...
from .serializer import TicketConsolidatedSerializer
...
class TicketConsolidatedView(viewsets.ModelViewSet):
    serializer_class = TicketConsolidatedSerializer
    queryset = TicketConsolidated.objects.all()
    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated]  # Permitir solo a usuarios autenticados crear tickets
    ...
    def update(self, request, *args, **kwargs):
        ...
        if new_status not in dict(TicketConsolidated.STATUS_OPTIONS):
            return Response(
    ...
    def user_tickets(self, request):
        ...
        user_tickets = TicketConsolidated.objects.filter(user=request.user)
    ...
    def assigned_tasks(self, request):
        ...
        tasks = TicketConsolidated.objects.filter(assigned_to=user)

        serializer = TicketConsolidatedSerializer(tasks, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def update_status(self, request, pk=None):
        ...
        serializer = TicketConsolidatedSerializer(ticket)
        return Response(serializer.data, status=status.HTTP_200_OK)