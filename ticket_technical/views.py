...
from .serializer import TicketTechnicalSerializer
...
class TicketTechnicalView(viewsets.ModelViewSet):
    serializer_class = TicketTechnicalSerializer
    queryset = TicketTechnical.objects.all()
    ...
    def update(self, request, *args, **kwargs):
        ...
        if new_status not in dict(TicketTechnical.STATUS_OPTIONS):
            return Response(
    ...
    def user_tickets(self, request):
        user_tickets = TicketTechnical.objects.filter(user=request.user)
    ...
    def assigned_tasks(self, request):
        ...
        tasks = TicketTechnical.objects.filter(assigned_to=user)

        serializer = TicketTechnicalSerializer(tasks, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def update_status(self, request, pk=None):
        ...
        serializer = TicketTechnicalSerializer(ticket)
        return Response(serializer.data, status=status.HTTP_200_OK)