...
class TicketFacilities(models.Model):
    ...
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='assigned_facilities_staff', on_delete=models.SET_NULL)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.custom_id:
            self.custom_id = f"FAC-{self.pk}" if self.pk else f"FAC-TEMP-{timezone.now().strftime('%Y%m%d%H%M%S')}"
        if not self.status:
            self.status = 'Enviado'
        ...
        if self.custom_id.startswith("FAC-TEMP") and self.pk:
            self.custom_id = f"FAC-{self.pk}"
...
class TicketFacilitiesFile(models.Model):
    fileupload = models.FileField(upload_to='ticketsfiles/', validators=[validate_file_extension])
    ticket = models.ForeignKey(TicketFacilities, related_name='files', on_delete=models.CASCADE)
